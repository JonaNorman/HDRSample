...
class EnvConfigImpl implements GLEnvConfig {
    ...
    private final int[] tempValue = new int[1];
    ...
    public EnvConfigImpl(EGLDisplay display, EGLConfig config) {
        ...
        for (ConfigValue configValue : configValueList) {
            if (EGL14.eglGetConfigAttrib(display, config, configValue.key, tempValue, 0)) {
                configValue.setValue(tempValue[0]);
            } else {
               GLEnvException.reset();
            }
        }
    }
