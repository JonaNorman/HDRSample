...
class AndroidVideoDemuxerImpl implements AndroidVideoDemuxer {
    ...
    @Override
    public synchronized void seekToPreviousSync(long timeUs) {
        if (!isAvailable()) {
            return;
        }
    ...
    @Override
    public synchronized void seekToClosestSync(long timeUs) {
        if (!isAvailable()) {
            return;
        }
    ...
    @Override
    public synchronized void seekToNextSync(long timeUs) {
        if (!isAvailable()) {
            return;
        }
